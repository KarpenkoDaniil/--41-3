...
def parse_data(log_data):
    """Парсит строки формата 'Time: 2024-12-11 19:21:47.91414, Byte's: 402;'."""
...
def plot_data(parsed_data):
    ...
    plt.title('График отношения даты к пакету данных', fontsize=26)
    plt.xlabel('Дата ', fontsize=24)
    plt.ylabel('Количество данных (пакет)', fontsize=24)
    plt.grid(True)
    plt.xticks(rotation=45)

    # Ограничение диапазона по оси Y (пример)
    plt.ylim(min(bytes_values) - 10, max(bytes_values) + 10)

    plt.tight_layout()
    plt.show()

# Загрузка данных из файла
file_path = 'Logger.txt'  # Укажите путь к вашему файлу
try:
